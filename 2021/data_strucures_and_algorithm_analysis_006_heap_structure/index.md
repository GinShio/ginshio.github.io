...
{{< figure src="/ox-hugo/complete-binary-tree-of-array-implement.svg" >}}
...
{{< figure src="/ox-hugo/min-heap-and-max-heap.svg" >}}
...
{{< figure src="/ox-hugo/binary-heap-insert.svg" >}}
...
{{< figure src="/ox-hugo/binary-heap-erase.svg" >}}
...
{{< figure src="/ox-hugo/4-ary-heap-example.svg" >}}
...
将任一结点 X 的 **零路径长** (null path length) \\(npl(X)\\) 定义为从 X 到一个布局有两个孩子的结点的最短路径长。因此具有 \\(degree = 0 \lor 1\\) 的结点 \\(npl = 0\\) ，而
\\(npl(null) = -1\\) ，任意结点的 NPL 比其所有孩子的 NPL 的最小值多 1。对于 heap 中的每一个结点 X，左孩子的 NPL 至少与右孩子的 NPL 相等，这样树的结构更偏向于向左子树添加深度，因此称之为 leftist heap。

{{< figure src="/ox-hugo/leftist-heap-example.svg" >}}
...
{{< figure src="/ox-hugo/leftist-heap-merge-example.svg" >}}
...
{{< figure src="/ox-hugo/skew-heap-merge-example.svg" >}}
...
\\[\left(\begin{aligned} k \\\ d \end{aligned}\right).\\] 如果将堆序施加于二项树上，并允许任意高度上最多一棵二项树，那么能够用二项树的集合唯一地表示任意大小的优先队列。例如，大小为 13 的优先队列可以用森林 \\(B\_3\\) 、
\\(B\_2\\) 、 \\(B\_0\\) 表示，将这种二项队列写作 \\(1101\\) 。

{{< figure src="/ox-hugo/binomial-queue-example.svg" >}}
...
合并两个二项队列在概念上很容易，对于两个同高度的二项树可以合并为更高的树，让值大的根成为值小的根的子树。有时合并后，可能出现三棵高度相同的树，在两个队列中各取一棵继续合并即可。直到没有高度相同的树为止，合并结束。因此上图 13 个结点的二项队列可看作是两个二项队列合并而来的。

{{< figure src="/ox-hugo/binomial-queue-merge-example.svg" >}}

插入可以看作合并的特殊情况，创建一棵单结点树，然后执行合并即可。删除操作由找到具有最小根的二项树来完成，将该树先从森林中移除，删除掉根后，拆解为新的二项队列，最后合并这两个队列即可。

还是以上图 13 个结点的二项队列为例，删除掉 12，将 \\(B\_3\\) 从森林中剥离，拆分为\\(B\_{30}\\)、\\(B\_{31}\\)、\\(B\_{32}\\)并构成新的队列，合并两个队列即可完成删除。

{{< figure src="/ox-hugo/binomial-queue-delete-min-example.svg" >}}
