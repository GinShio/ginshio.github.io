...
\begin{tikzpicture}
  \begin{scope}[
    ...
    \node[protochain node] (transport)   {SCTP};
  ...
  \begin{scope}[
    ...
    \path (user-buffer.south) -- node[align=right,above left]{\tiny write}
    ++($(kernel-buffer.north west)-(user-buffer.south west)$);
    \path ($(kernel-buffer.south west)!0.5!(kernel-buffer.south)$) --
    ++($(internet-buffer)-(kernel-buffer.south west)$) coordinate(temp-coor);
    \path (temp-coor) ++($(kernel-buffer.south)-(kernel-buffer.north)$)
