...
\begin{tikzpicture}
  \begin{scope}[
    ...
    \node[protochain node] (transport)   {UDP};
  ...
  \begin{scope}[
    ...
    \node[buffer node,dashed] (kernel-buffer) {socket send buffer (\textbf{SO\_SNDBUF})};
    \coordinate[buffer coor] (internet-buffer);
    \coordinate[buffer coor] (link-buffer);
    \path[draw,->] (user-buffer.south) -- node[align=right,above left]{\tiny
      sendto} ++($(kernel-buffer.north west)-(user-buffer.south west)$);
    \path[draw,->] ($(kernel-buffer.south west)!0.5!(kernel-buffer.south)$) --
    node[align=left,right]{{\tiny UDP datagram}}
    ++($(internet-buffer)-(kernel-buffer.south west)$) coordinate(temp-coor);
    \path[draw,->] (temp-coor) ++($(kernel-buffer.south)-(kernel-buffer.north)$)
    -- node[align=left,right]{\tiny MTU-sized IPv4 or IPv6 packets}
    ++($(link-buffer)-(internet-buffer)$);
  \end{scope}
